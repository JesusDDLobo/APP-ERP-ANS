...
import com.example.app_ans.vehicles.ui.VehicleDetailActivity;
...
/** Home screen that reacts to auth flags (password change, Google status). */
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Remove the listener that was pushing the whole root down
    ...
    private void handleTaskRedirect(Intent intent) {
        if (intent != null && (intent.hasExtra("TASK_ID") || intent.hasExtra("task_id"))) {
            ...
            if (taskId != -1) {
                ...
                // Clear extra from Main so it doesn't fire again on rotation
    ...
    private void setupUi() {
        binding.tasksButton.setOnClickListener(v -> startActivity(new Intent(this, com.example.app_ans.TasksActivity.class)));
        binding.hotelButton.setOnClickListener(v -> startActivity(new Intent(this, com.example.app_ans.HotelActivity.class)));
        binding.vehicleButton.setOnClickListener(v -> startActivity(new Intent(this, com.example.app_ans.vehicles.ui.VehicleDetailActivity.class)));

        // Setup notification button widget
        setupNotificationWidget();

        binding.gpsTestContainer.setOnClickListener(v -> {
            if (com.example.app_ans.core.utils.LocationHelper.hasLocationPermissions(this)) {
                testGps();
            } else {
        ...
        com.example.app_ans.core.ui.NavbarUtils.setupNavbar(this, null, this::showNotificationsBottomSheet, () -> authViewModel.logout(this));
    }

    private void setupNotificationWidget() {
        // Logic to initialize the notification widget
    }
    ...
    private void observeViewModel() {
        ...
        // This LiveData now correctly receives the signal from the ViewModel
        authViewModel.getLogoutResult().observe(this, success -> {
            if (Boolean.TRUE.equals(success)) {
                // Navigate to AuthActivity after successful logout
        ...
        authViewModel.getPasswordSetupResult().observe(this, success -> {
            if (Boolean.TRUE.equals(success)) {
                binding.passwordBanner.setVisibility(View.GONE);
                Toast.makeText(this, "Contraseña actualizada", Toast.LENGTH_SHORT).show();
            }
        });
        authViewModel.getGoogleAckResult().observe(this, success -> {
            if (Boolean.TRUE.equals(success)) {
                binding.googleStatusBanner.setVisibility(View.GONE);
            }
        });
    }

    private void testGps() {
        binding.gpsStatusText.setText("Obteniendo...");
        com.example.app_ans.core.utils.LocationHelper.getCurrentLocation(this, new com.example.app_ans.core.utils.LocationHelper.LocationCallback() {
            @Override
            public void onSuccess(double latitude, double longitude) {
                binding.gpsStatusText.setText(String.format(java.util.Locale.US, "%.4f, %.4f", latitude, longitude));
            }

            @Override
            public void onFailure(String message) {
                binding.gpsStatusText.setText("Error GPS");
    ...
    private void handleIncomingFlags() {
        ...
        if (mustChangePassword) {
            binding.passwordBanner.setVisibility(View.VISIBLE);
        ...
        if (googleStatus != null && !"ok".equalsIgnoreCase(googleStatus)) {
            binding.googleStatusBanner.setVisibility(View.VISIBLE);
            binding.googleStatusMessage.setText(
                    getIntent().getStringExtra("google_services_message")
            );
            binding.googleStatusAcknowledge.setVisibility(requiresAck ? View.VISIBLE : View.GONE);
